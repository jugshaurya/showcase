...
- If the current element is of higher precedence(heavy one), push to stack
...
- `Infix to prefix algo`
  ...
  - just traverse the infux string from reverse and return the reverse of what you get, rest algo is the same as above.
...
- Collisions are resolved by:-
  - Open Addressing or Closed hashing
    - Linear Probing (hf_i(n) = (hf(n) + i) mod m); [hf_i being calculating hf ith time; m is table size]
      ...
      - Time for unsuccessful search = 1/(1-LF) on an average.
    - Quadratic Probing (hf_i(n) = (hf(n) + a*i*i + b\*i)) mod m, typically a=1 & b=0
      ...
      - Secondary Clustering: - In case of Linear Probing, If two keys are mapped to same starting location in a hash table then both follow the same path unnecessarily in a linear fashion as a result of which search time increases. To avoid this problem, we use Double hashing.
